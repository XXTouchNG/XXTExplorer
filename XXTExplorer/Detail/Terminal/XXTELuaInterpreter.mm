...
int lua_pushobject (lua_State *L, id object) {
    if ([object isKindOfClass:[NSString class]]) {
        ...
        //NSLog(@"XXTELuaInterpreter: Warning: Unsupported class %@ in `lua_pushobject`", [object class]);
...
id lua_toobject (lua_State *L, int idx) {
    ...
    switch (type) {
        ...
        case LUA_TNIL:
        default:
            NSLog(@"XXTELuaInterpreter: Warning: Unsupported type `%@` in `lua_toobject`", lua_stype(L, idx));
            return nil;
    }
}
...
int _runSelector (lua_State *L) {
    ...
    XXTELuaArgumentType returnType = (XXTELuaArgumentType)lua_tonumber(L, lua_upvalueindex(4)); // Return type
    ...
    if (argTypes.count != argCount) {
        NSLog(@"XXTELuaInterpreter: Warning: Wrong number of arguments in C closure");
    }
    ...
    if (argTypes) for (int i = 0; i < argCount; i++) {
        switch ([[argTypes objectAtIndex:i] intValue]) {
            ...
            case XXTELuaArgumentTypeString: {
            ...
            case XXTELuaArgumentTypeBoolean: {
                BOOL b = lua_toboolean(L, -argCount + i);
                [inv setArgument:&b atIndex:2+i];
            }
                
            case XXTELuaArgumentTypeObject:
            case XXTELuaArgumentTypeLightObject: {
    ...
    switch (returnType) {
        ...
        case XXTELuaArgumentTypeMultiple: {
            __unsafe_unretained id ret; [inv getReturnValue:&ret];
            if ([ret isKindOfClass:[NSArray class]]) {
                for (id obj in ret) lua_pushobject(L, obj);
                return (int)[(NSArray*)ret count];
            } else {
                NSLog(@"XXTELuaInterpreter: Warning: Function registered return type is `multiple`, but the returned object isn't an NSArray");
                return 0;
            }
        }

        case XXTELuaArgumentTypeNone:
        default:
            return 0;
    }
}

@interface XXTELuaInterpreter() {
...
- (void) _registerSelector:(SEL)selector target:(id)target name:(NSString *)name returnType:(XXTELuaArgumentType)returnType argumentTypesArray:(NSArray*)argumentTypesArray;

@end

@implementation XXTELuaInterpreter
...
- (BOOL) load:(NSString *)filepath {
    ...
    if (ret) {
        NSLog(@"XXTELuaInterpreter: Error loading file [%@]: %s", filepath, lua_tostring(state, -1));
        lua_showstack(state);
        return NO;
    }
...
- (BOOL) run {
    ...
    if (ret) {
        NSLog(@"XXTELuaInterpreter: Error: %s", lua_tostring(state, -1));
        return NO;
    }
...
- (id) call:(NSString *)fname withArguments:(id)args, ... {
    ...
    if (!lua_isfunction(state, -1)) {
        NSLog(@"XXTELuaInterpreter: Error: Tried to run non-existing function `%@`", fname);
        lua_pop(state, 1);
        return nil;
    }
    ...
    if (lua_pcall(state, argCount, 0, 0) != 0) {
        NSLog(@"XXTELuaInterpreter: Error: Function `%@`: %@", fname, [NSString stringWithCString:lua_tostring(state, -1) encoding:NSUTF8StringEncoding]);
    }
    return nil;
}
...
- (void) registerSelector:(SEL)selector target:(id)target name:(NSString *)name {
    [self _registerSelector:selector target:target name:name returnType:XXTELuaArgumentTypeNone argumentTypesArray:nil];
}
...
- (void) registerSelector:(SEL)selector target:(id)target name:(NSString *)name argumentTypes:(int)count, ... {
    ...
    for (int i = 0; i < count; i++) {
        [argumentTypesArray addObject:[NSNumber numberWithInt:va_arg(argumentTypes, XXTELuaArgumentType)]];
    }
    va_end(argumentTypes);
    
    [self _registerSelector:selector target:target name:name returnType:XXTELuaArgumentTypeNone argumentTypesArray:argumentTypesArray];
}

- (void) registerSelector:(SEL)selector target:(id)target name:(NSString *)name returnType:(XXTELuaArgumentType)returnType {
    [self _registerSelector:selector target:target name:name returnType:returnType argumentTypesArray:nil];
}

- (void) registerSelector:(SEL)selector target:(id)target name:(NSString *)name returnType:(XXTELuaArgumentType)returnType argumentTypes:(int)count, ... {
    // Copy argument types into an array for use with _runSelector
    NSMutableArray *argumentTypesArray = [NSMutableArray array];
    va_list argumentTypes;
    va_start(argumentTypes, count);
