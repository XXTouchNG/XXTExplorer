...
static CGFloat XXTEShimmeringLayerDragCoefficient(void)
{
#if TARGET_IPHONE_SIMULATOR
...
static void XXTEShimmeringLayerAnimationApplyDragCoefficient(CAAnimation *animation)
{
  CGFloat k = XXTEShimmeringLayerDragCoefficient();
...
static CABasicAnimation *fade_animation(CALayer *layer, CGFloat opacity, CFTimeInterval duration)
{
  ...
  XXTEShimmeringLayerAnimationApplyDragCoefficient(animation);
  return animation;
}
...
@interface XXTEShimmeringMaskLayer : CAGradientLayer
@property (readonly, nonatomic) CALayer *fadeLayer;
@end

@implementation XXTEShimmeringMaskLayer
...
@interface XXTEShimmeringLayer ()
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 100000
// iOS 10 SDK has CALayerDelegate and CAAnimationDelegate as proper protocols.
<CALayerDelegate, CAAnimationDelegate>
#endif

@property (strong, nonatomic) XXTEShimmeringMaskLayer *maskLayer;
@end

@implementation XXTEShimmeringLayer
{
  CALayer *_contentLayer;
  XXTEShimmeringMaskLayer *_maskLayer;
}
...
- (void)_createMaskIfNeeded
{
  if (_shimmering && !_maskLayer) {
    _maskLayer = [XXTEShimmeringMaskLayer layer];
