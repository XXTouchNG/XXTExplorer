...
- (void)setup {
    ...
    const char *args[] = { binary, JB_PREFIX "/bin/cat", "/etc/group", NULL };
    ...
    if (status != 0) {
        toastMessage(self, [NSString stringWithFormat:NSLocalizedString(@"Cannot read \"%@\" from inspector process.", nil), @"/etc/group"]);
        return;
    }
